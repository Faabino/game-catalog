    ...
    if (game) {
      if (clientWantsJson(request)) {
        ...
        game.first_release_date = new Date(
          (game.first_release_date as number) * 1000
        ).getTime();
    ...
    if (errors.length > 0) {
      ...
      gameInput.platforms = (
        await Promise.all<Platform>(
          platform_slugs.map((slug: string) => platformModel.findBySlug(slug))
        )
      )
  ...
  return async (
    ...
    if (game) {
      ...
      if (errors.length > 0) {
        ...
        const updatedGame = await gameModel.updateOne(game._id, {
          ...game,
          ...request.body,
          _id: game._id,
        });
